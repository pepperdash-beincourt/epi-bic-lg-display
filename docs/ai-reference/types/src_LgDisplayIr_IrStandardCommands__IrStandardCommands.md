...
| Source file | [`src/LgDisplayIr/IrStandardCommands.cs`](../../../src/LgDisplayIr/IrStandardCommands.cs) |
| Language | C# |
| Declaration | `class IrStandardCommands` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Plugins.Lg.Display` |

## What

`IrStandardCommands` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `GetCommandValue`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `IrStandardCommands`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
