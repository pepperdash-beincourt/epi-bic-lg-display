...
| Source file | [`src/LgDisplayPropertiesConfig.cs`](../../../src/LgDisplayPropertiesConfig.cs) |
| Language | C# |
| Declaration | `class LgDisplayPropertiesConfig` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Plugins.Lg.Display` |

## What

`LgDisplayPropertiesConfig` is a configuration or data-transfer shape used to deserialize and validate structured input. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `LgDisplayPropertiesConfig`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
