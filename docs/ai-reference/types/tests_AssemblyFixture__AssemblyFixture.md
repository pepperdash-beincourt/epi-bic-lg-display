...
| Source file | [`tests/AssemblyFixture.cs`](../../../tests/AssemblyFixture.cs) |
| Language | C# |
| Declaration | `class AssemblyFixture` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Plugins.Lg.Display.Tests` |

## What

`AssemblyFixture` is a test-support or verification type that protects a defined behavior from regression. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `FindFactoryTypes`, `FindSourceForClass`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `AssemblyFixture`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
