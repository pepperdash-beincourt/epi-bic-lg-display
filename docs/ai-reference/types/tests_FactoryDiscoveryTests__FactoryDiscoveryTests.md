...
Public methods declared in this source file include: `Assembly_Loads_Successfully`, `Assembly_Name_Is_EpiDisplayLg`, `Factory_Count_Is_Two`, `Factory_Exists_ByName`, `All_Factories_Have_Parameterless_Constructor`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
