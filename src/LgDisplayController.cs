...
namespace PepperDash.Essentials.Plugins.Lg.Display
{
        ...
        IDPad, INumericKeypad, ITransport, IChannel, IColor
    {
        ...
        /// <summary>
        /// Sends an LG SIC "mc" (Remote Control Key Action) command - the RS232 equivalent of
        /// the IR driver's SendIrCommand. See LgKeyCodes.cs: values are placeholders pending
        /// verification against LG's official SIC reference before this ships.
        /// </summary>
        public void SendKeyCommand(string keyCode)
        {
            if (string.IsNullOrEmpty(keyCode))
            {
                this.LogError("SendKeyCommand: key code is null or empty");
                return;
            }

            SendData(string.Format("mc {0} {1}", Id, keyCode));
        }
        ...
        public void Up(bool pressRelease)
        {
            ...
            SendKeyCommand(LgKeyCodes.DpadUp);
        }

        public void Down(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.DpadDown);
        }

        public void Left(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.DpadLeft);
        }

        public void Right(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.DpadRight);
        }

        public void Select(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.DpadSelect);
        }

        public void Menu(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Menu);
        }
        ...
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Exit);
        }
        ...
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Play);
        }

        public void Pause(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Pause);
        }

        public void Rewind(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Rewind);
        }

        public void FFwd(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.FastForward);
        }
        ...
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.ChannelUp);
        }

        public void ChannelDown(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.ChannelDown);
        }

        public void LastChannel(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Last);
        }

        public void Guide(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Guide);
        }
        ...
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.FuncRed);
        }

        public void Green(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.FuncGreen);
        }

        public void Yellow(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.FuncYellow);
        }

        public void Blue(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.FuncBlue);
        }

        #endregion


        #region Media app buttons

        // Netflix/Prime/Disney/Samsung TV Plus are remote-key presses over "mc", not physical
        // inputs - LG's "xb" input-select command (see LgInput.Select() in LGInputs.cs) only
        // covers real HDMI/DisplayPort-style sources, not app shortcuts, so these are NOT added
        // to the Inputs/routing dictionary the way the IR driver's Netflix/PrimeVideo are.

        public void InputNetflix(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Netflix);
        }

        public void InputPrimeVideo(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.PrimeVideo);
        }

        public void InputDisney(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Disney);
        }

        public void InputSamsungTvPlus(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.SamsungTvPlus);
        }
        ...
        // Home, Back, Enter have no dedicated core DeviceTypeInterfaces member (IDPad only has
        // Menu/Exit). Exposed as plain methods, same precedent as the input-select methods above.

        public void Home(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Home);
        }

        public void Back(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Back);
        }

        public void Enter(bool pressRelease)
        {
            if (pressRelease) return;
            SendKeyCommand(LgKeyCodes.Enter);
        }
